...
// async function fillMissingWhoisData(tableRows){

// 	tableRows.forEach((tableRow, index, array) => {

// 		let apiUrl = 'ipinfo.io'
// 		let ipinfo_token = process.env.IPINFO_TOKEN;
// 		let path = `${tableRow.visitor_ip}?token=${ipinfo_token}`
// 		if (tableRow.whoisdata === null || tableRow.whoisdata === undefined ) {
// 			https.get(`https://${apiUrl}/${path}`, res => {
// 			let data = [];
...
// 			res.on('end', () => {
// 				const whoisdata = Buffer.concat(data);
// 				console.log('Response ended: ' + whoisdata );
				
// 				pool
// 				.query("UPDATE visitors_list SET whois_data = $1 WHERE visitor_ip = $2;",[ whoisdata , tableRow.visitor_ip ])
// 				.then(answer => console.log('update ok!') )
// 				.catch(err =>
// 					setImmediate(() => {
// 						throw err
// 					})
